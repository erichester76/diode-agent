...
from diode_prometheus.client import Client
from diode_prometheus.discovery import discover_device_driver, supported_drivers
from diode_prometheus.parser import (
    Diode,
    DiscoveryConfig,
    prometheus,
    Policy,
...
from diode_prometheus.version import version_semver
...
def run_driver(info: prometheus, config: DiscoveryConfig):
    """
    ...
    if info.driver is None:
        ...
        raise Exception(
            ...
            f"{supported_drivers}.\nHINT: If '{info.driver}' is a prometheus community driver, try to perform the following command:"
            f"\n\n\tpip install prometheus-{info.driver.replace('_', '-')}\n"
        )
...
def main():
    """
    Main entry point for the Diode prometheus Agent CLI.

    Parses command-line arguments and starts the agent.
    """
    parser = argparse.ArgumentParser(description="Diode Agent for prometheus")
    parser.add_argument(
        "-V",
        "--version",
        action="version",
        version=f"Diode Agent version: {version_semver()}, prometheus version: {version('prometheus')}, "
        f"Diode SDK version: {SdkVersion.version_semver()}",
        help="Display Diode Agent, prometheus and Diode SDK versions",
    )
    parser.add_argument(
        "-c",
        "--config",
